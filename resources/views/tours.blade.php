 ...
 <div class="row">
    ...
    <div class="container-fluid content">
        <div class="row color">
            ...
            <div class="col-sm-4 sec3" id="map" style="height: 92vh;"></div>
            <div class="col-sm">
        ...
        <script>
          ...
          function initMap() {
              ...
              @foreach($tours as $tour)
                ...
                var myLatLng_{{ $tour->id }} = { lat: {{ $tour->latitude }}, lng: {{ $tour->longitude }} };
                var iconUrl = '{{ asset('images/red.PNG') }}'; // Set the default icon URL

                var marker_{{ $tour->id }} = new google.maps.Marker({
                  position: myLatLng_{{ $tour->id }},
                  icon: iconUrl,
                  map: map
                });
                ...
                marker_{{$tour->id}}.addListener('click', function(){
                  var businessName ="{{$tour->title}}";
                  var businessId = "{{$tour->id}}";
                  var latvalue ="{{$tour->latitude}}";
                  var longvalue ="{{$tour->longitude}}";
                  ...
                  currentMarker = marker_{{$tour->id}};

                  var attractionDetails = document.getElementById('details');
                  attractionDetails.innerHTML = ''; // Clear previous package details

                        var attractionElement = document.createElement('div');
                        attractionElement.innerHTML = `
                            <h5>Organized By {{$tour->fname}} {{$tour->lname}}</h5>
                            <h6>{{$tour->email}} | {{$tour->contact}}</h6>
                            <hr>
                            <div class="tourbox">
                            <div class="row">
                            <div class="col-sm-6">
                            <img src="{{asset($tour->img_one)}}" class="img-fluid">
                            </div>
                            <div class="col-sm-6">
                            <img src="{{asset($tour->img_two)}}" class="img-fluid">
                            </div>
                            </div>
                            <br>
                            <div class="row">
                            <div class="col-sm-6">
                            <img src="{{asset($tour->img_three)}}" class="img-fluid">
                            </div>
                            <div class="col-sm-6">
                            <img src="{{asset($tour->img_four)}}" class="img-fluid">
                            </div>
                            </div><br>
                            <h5>Package Price: {{$tour->price}} {{$tour->currency}}</h5>
                            <hr>
                            <p style="white-space: pre-wrap;"><i>{{$tour->details}}</I></p>
                            <br>
                            <p><b>If you like this tour package or if you want to select this tour package with additional changes, Please contact me via e-mail or phone </b></p>
